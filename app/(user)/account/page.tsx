...
export default function AccountPage() {
    ...
    const sectionMeta: Record<string, { title: string; subtitle: string }> = {
        profile: {
            title: 'Profile',
            subtitle: 'Update your public profile and business details.',
        },
        security: {
            title: 'Security',
        ...
        billing: {
            title: 'Billing',
            subtitle: 'Manage your subscriptions and payment history.',
        },
    };

    const meta = sectionMeta[activeTab] ?? sectionMeta['profile'];
    ...
    return (
        <div className="space-y-8 pb-20">
            ...
            <div>
                ...
                <p className="text-slate-500 font-medium text-sm mt-1">{meta.subtitle}</p>
            </div>
            ...
            {/* ── Billing Tab ──────────────────────────────────────────────── */}
            {activeTab === 'billing' && isVerified && <BillingSettings />}
            {activeTab === 'billing' && !isVerified && (
                <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center">
                    <CreditCard className="mx-auto h-10 w-10 text-slate-300 mb-4" />
                    <h3 className="font-bold text-slate-700 mb-1">Billing Unavailable</h3>
                    <p className="text-sm text-slate-500">Billing features are only available to verified members.</p>
                </div>
            )}
        </div>
    );
